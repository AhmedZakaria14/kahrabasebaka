      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <ScrollReveal animation="fade-up" className="text-center max-w-3xl mx-auto mb-12">
          ...
          <div className="relative max-w-md mx-auto mt-6">
            <input
              ...
              className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-300 rounded-2xl text-xs sm:text-sm focus:bg-white focus:border-blue-600 outline-none shadow-sm transition"
            />
          ...
          <ScrollReveal animation="fade-up" delay={0.1} className="flex items-center justify-center gap-2 mb-8 flex-wrap">
            {districtsData.map((zone) => (
              <button
                ...
                className={`px-4 sm:px-6 py-2.5 rounded-xl font-bold text-xs sm:text-sm transition-all shadow-sm flex items-center gap-2 ${
        ...
        <ScrollReveal animation="scale-up" delay={0.15}>
          <div className="bg-slate-50 rounded-3xl p-6 sm:p-8 border border-slate-200/80 shadow-sm">
            <div className="flex items-center justify-between gap-4 mb-6 pb-4 border-b border-slate-200 flex-wrap">
              <div>
              ...
              <AnimatePresence mode="popLayout">
                {filteredDistricts.map((district) => (
                  <motion.button
                    ...
                    className="group p-3 bg-white hover:bg-blue-600 text-slate-800 hover:text-white rounded-xl border border-slate-200 hover:border-blue-600 shadow-sm hover:shadow-md transition-all text-right flex items-center justify-between"
                  >
                    <span className="text-xs sm:text-sm font-semibold truncate">
                      حي {district}
                    </span>
                    <MapPin className="w-3.5 h-3.5 text-slate-400 group-hover:text-amber-300 shrink-0" />
                  </motion.button>
                ))}
              </AnimatePresence>
            </motion.div>
